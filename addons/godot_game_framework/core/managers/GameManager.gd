...
func _load_state_definitions() -> void:
	if states_config_path.is_empty():
		GGF.log().error("GameManager", "States config path is empty. Cannot initialize state machine.")
		return

	if not ResourceLoader.exists(states_config_path):
		GGF.log().error("GameManager", "States config resource not found: " + states_config_path + ". Cannot initialize state machine.")
		return
	...
	if not config_resource.has_method("validate") or not config_resource.has_method("get_state") or not config_resource.has_method("has_state"):
		GGF.log().error("GameManager", "Loaded resource is not a valid GameStateMachineConfig. Cannot initialize state machine.")
		return

	# Validate the configuration
	if not config_resource.validate():
		GGF.log().error("GameManager", "State machine configuration validation failed. Cannot initialize state machine.")
		return
...
func change_state(new_state: String) -> void:
	...
	if current_state_def != null:
		...
		if not allowed_transitions.is_empty() and not allowed_transitions.has(new_state):
			GGF.log().warn("GameManager", "Transition from '" + current_state + "' to '" + new_state + "' is not allowed")
			return
...
func _call_state_callback(callback_name: String, state_name: String) -> void:
	...
	if has_method(callback_name):
		...
		GGF.log().warn("GameManager", "State callback method '" + callback_name + "' not found for state '" + state_name + "'")
...
func change_scene(scene_path: String, transition_type: String = "") -> void:
	...
	GGF.log().info("GameManager", "Changing scene to: " + scene_path + (" with transition: " + transition_type if transition_type else ""))
