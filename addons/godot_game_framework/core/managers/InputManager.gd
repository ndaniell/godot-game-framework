...
func is_action_pressed(action: String) -> bool:
	if not InputMap.has_action(action):
		GGF.log().warn("InputManager", "Action does not exist: " + action)
		return false
...
func get_action_vector(
	...
	if (
		not InputMap.has_action(negative_x)
		or not InputMap.has_action(positive_x)
		or not InputMap.has_action(negative_y)
		or not InputMap.has_action(positive_y)
	):
		GGF.log().warn("InputManager", "One or more actions do not exist")
		return Vector2.ZERO
...
func remap_action(action: String, event: InputEvent, remove_old: bool = true) -> bool:
	if not InputMap.has_action(action):
		GGF.log().error("InputManager", "Cannot remap non-existent action: " + action)
		return false
...
func reset_action(action: String) -> bool:
	if not InputMap.has_action(action):
		GGF.log().error("InputManager", "Cannot reset non-existent action: " + action)
		return false
...
func _save_input_remaps() -> void:
	...
	if file == null:
		GGF.log().warn("InputManager", "Failed to save input remaps")
		return
...
func _load_input_remaps() -> void:
	...
	if json.parse(json_string) != OK:
		GGF.log().warn("InputManager", "Failed to parse input remaps")
		return
