...
func host(port: int = -1) -> bool:
	...
	if err != OK:
		_peer = null
		GGF.log().error("NetworkManager", "Failed to create server on port " + str(port) + " (err=" + str(err) + ")")
	...
	GGF.log().info("NetworkManager", "Successfully hosting on port " + str(port) + " (max clients: " + str(max_clients) + ")")
...
func join(ip: String, port: int = -1) -> bool:
	...
	if err != OK:
		_peer = null
		GGF.log().error("NetworkManager", "Failed to create client for " + ip + ":" + str(port) + " (err=" + str(err) + ")")
...
func send_session_event_to_peer(peer_id: int, event_name: StringName, data: Dictionary = {}) -> void:
	...
	GGF.log().trace("NetworkManager", "Sending session event '" + event_name + "' to peer " + str(peer_id))
