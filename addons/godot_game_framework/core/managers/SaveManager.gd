...
func _ready() -> void:
	...
	GGF.log().info("SaveManager", "SaveManager initializing...")
...
func save_game(slot: int = 0, metadata: Dictionary = {}) -> bool:
	...
	GGF.log().info("SaveManager", "Saving game to slot " + str(slot))
...
func load_game(slot: int = 0) -> bool:
	...
	GGF.log().info("SaveManager", "Loading game from slot " + str(slot))
	...
	if parse_error != OK:
		GGF.log().error("SaveManager", "Failed to parse save file JSON: " + file_path)
