...
func _ready() -> void:
	...
	GGF.log().info("ResourceManager", "ResourceManager initializing...")
...
func load_resource(resource_path: String, use_cache: bool = true) -> Resource:
	if resource_path.is_empty():
		GGF.log().error("ResourceManager", "Cannot load empty resource path")
		return null
	...
	if use_cache and enable_caching and _resource_cache.has(resource_path):
		...
		if cached_resource != null:
			...
			GGF.log().trace("ResourceManager", "Cache hit for resource: " + resource_path)
			_on_resource_loaded_from_cache(resource_path, cached_resource)
			return cached_resource

	# Load resource
	if not ResourceLoader.exists(resource_path):
		GGF.log().error("ResourceManager", "Resource does not exist: " + resource_path)
		return null

	GGF.log().debug("ResourceManager", "Loading resource: " + resource_path)
	var resource := load(resource_path) as Resource
	if resource == null:
		GGF.log().error("ResourceManager", "Failed to load resource: " + resource_path)
		return null

	GGF.log().debug("ResourceManager", "Successfully loaded resource: " + resource_path)
...
func load_resource_async(resource_path: String, use_cache: bool = true) -> Resource:
	...
	if loader == null:
		GGF.log().error("ResourceManager", "Failed to start async load: " + resource_path)
		return null
	...
	if status != ResourceLoader.THREAD_LOAD_LOADED:
		GGF.log().error("ResourceManager", "Async load failed: " + resource_path)
	else:
		resource = ResourceLoader.load_threaded_get(resource_path) as Resource
		if resource == null:
			GGF.log().error(
				"ResourceManager", "Failed to get async loaded resource: " + resource_path
			)
		else:
			# Cache resource if enabled
			if use_cache and enable_caching:
...
func preload_resource(resource_path: String) -> Resource:
	if resource_path.is_empty():
		GGF.log().error("ResourceManager", "Cannot preload empty resource path")
		return null
