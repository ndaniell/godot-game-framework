...
func assert_not_equal(actual: Variant, expected: Variant, message: String = "") -> bool:
	if actual == expected:
		var msg := (
			message if not message.is_empty() else "Expected values to differ, both are %s" % actual
		)
		_fail_test(msg)
		return false
	return true
...
func assert_almost_equal(
	...
	if diff > tolerance:
		var msg: String = (
			...
			else "Expected %s, got %s (diff: %s)" % [expected, actual, diff]
		)
		_fail_test(msg)
		return false
	return true
...
func run_test_suite(
	...
	for test_name in tests:
		...
		if not test_func.is_valid():
			push_warning(
				(
					"TestFramework: Invalid test function for: "
					+ test_name
					+ " (type: "
					+ str(typeof(test_value))
					+ ")"
				)
			)
			continue
...
func run_test_suite_object(test_suite: RefCounted) -> Dictionary:
	...
	return run_test_suite(test_suite.get_suite_name(), test_suite.get_tests(), setup, teardown)
...
func run_registered_suites(registry: RefCounted) -> Dictionary:
		...
		if tests_raw is Dictionary:
			for test_name in tests_raw:
				...
				if test_value is Callable:
					...
					push_warning(
						"TestFramework: Cannot create Callable from method name: " + test_name
					)
				else:
					# Try to cast
					...
					if callable.is_valid():
						...
						push_warning(
							(
								"TestFramework: Invalid test value for: "
								+ test_name
								+ " (type: "
								+ str(typeof(test_value))
								+ ")"
	...
	print(
		"Success rate: ",
		"%.1f%%" % (float(total_passed) / float(total_tests) * 100.0) if total_tests > 0 else "0.0%"
	)
	print("=".repeat(60))
